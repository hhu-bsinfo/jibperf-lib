...
class JniUtil {
    ...
    private static boolean DETECTOR_JNI_LOADED = false;

    /**
     * Load the shared object 'libdetectorJNI.so' from the JAR-file.
     */
    public static void loadIbPerfLibJNI() {
        if(DETECTOR_JNI_LOADED) {
            return;
        }

        try {
            JniUtil.loadNativeLibraryFromJar("/libdetectorJNI.so");
            DETECTOR_JNI_LOADED = true;
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
     ...
     */
     private static void loadNativeLibraryFromJar(String path) throws IOException {
        File tmpDir = Files.createTempDirectory("jdetector-native").toFile();
        tmpDir.deleteOnExit();
